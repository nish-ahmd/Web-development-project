...
export const StdForm = () => {
    ...
    return ( <
        ...
        id = "Fullname"
        value = { fullName }
        required onChange = { e => setFullname(e.target.value) } > < /input> <
        ...
        value = { stream } > {
            subs.map((subs) => ( <
                ...
                value = { subs.value } > { subs.label } < /option>
            ))
        } <
        /select><br/ >
        <
        ...
        value = { batches } > {
            batch.map((batch) => ( <
                option key = { Math.random().toString(36).substr(2, 9) }
                value = { batch.value } > { batch.label } < /option>
            ))
        } <
        /select><br/ >
        <
        ...
        button id = "sign"
        onClick = { submitForm } > Sign up < /button><br/ >
        <
        /form> < /
        section > <
        /div> < /
